...
function computeDerivedStatus(
  ...
  if (chapters.some((c) => c.moderationStatus === 'rejected'))
    return 'rejected';
  if (chapters.some((c) => c.moderationStatus === 'needs_admin_review'))
    return 'needs_admin_review';
  if (chapters.some((c) => c.moderationStatus === 'pending_moderation'))
    return 'pending_moderation';
  if (
    chapters.length > 0 &&
    chapters.every((c) => c.moderationStatus === 'approved')
  )
    return 'approved';
  return 'draft';
}
...
export function computeWorkAggregateFromChapters(
  ...
  const moderationUpdatedAt = maxDate(
    chapters.map(
      (chapter) =>
        safeDate(chapter.moderationUpdatedAt) ?? safeDate(chapter.updatedAt),
    ),
  );
  ...
  const contributing = chapters.filter(
    (c) => c.moderationStatus === derivedStatus,
  );
  ...
  if (representativePool.length > 0) {
    ...
    if (chosen) {
      representative = {
        chapterId:
          chosen.chapter?._id?.toString?.() ??
          String(chosen.chapter?._id ?? ''),
