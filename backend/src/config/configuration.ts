...
export default () => ({
  ...
  auth: {
    ...
    jwtExpiresInSeconds:
      Number.parseInt(
        (process.env.JWT_EXPIRES_IN_SECONDS || '604800').trim(),
        10,
      ) || 604800,
  ...
  moderation: {
    baseUrl: (
      process.env.MODERATION_BASE_URL || 'http://localhost:8000'
    ).trim(),
    requestTimeoutMs:
      Number.parseInt(
        (process.env.MODERATION_REQUEST_TIMEOUT_MS || '6000').trim(),
        10,
      ) || 6000,
    safeConfidenceThreshold:
      Number.parseFloat(
        (process.env.MODERATION_SAFE_CONFIDENCE_THRESHOLD || '0.88').trim(),
      ) || 0.88,
    unsafeConfidenceThreshold:
      Number.parseFloat(
        (process.env.MODERATION_UNSAFE_CONFIDENCE_THRESHOLD || '0.88').trim(),
      ) || 0.88,
  },
});
